...
def banner():
 ...
 {xanhla}██████╗██╗  ██╗██╗      ███╗   ███╗██╗   ██╗███╗   ███╗
{trang}██╔════╝██║  ██║██║      ████╗ ████║██║   ██║████╗ ████║
{xanhla}██║     ███████║██║█{trang}█{xanhla}█{trang}█{xanhla}█╗██╔████╔██║██║   ██║██╔████╔██║
{trang}██║     ██╔══██║██║╚════╝██║╚██╔╝██║██║   ██║██║╚██╔╝██║
{xanhla}╚██████╗██║  ██║██║      ██║ ╚═╝ ██║╚██████╔╝██║ ╚═╝ ██║
 {trang}╚═════╝╚═╝  ╚═╝╚═╝      ╚═╝     ╚═╝ ╚═════╝ ╚═╝     ╚═╝
\033[1;33m╠═══════════════════════════════════════════════════════╣
\033[1;33m║\033[1;34m▶ BY TOOL : {xanhla}CHÍ MUM                                    \033[1;33m║
\033[1;33m║\033[1;34m▶ ZALO : {vang}0834617174                                    \033[1;33m║
\033[1;33m║\033[1;34m▶ FACEBOOK: {vang}https://www.facebook.com/CODETOOLBYCHIMUM  \033[1;33m║
\033[1;33m║\033[1;34m▶ YOUTUBE: {vang}https://www.youtube.com/@TOOLBYCHIMUM       \033[1;33m║
\033[1;33m║\033[1;34m▶ \033[1;35mTOOL AUTO SPAM + CALL                                \033[1;33m║
\033[1;33m╚═══════════════════════════════════════════════════════╝{trang}
"""
    for x in banner:
        sys.stdout.write(x)
...
# /* Copyright © 27/07/2024 : Developer Chí Mum */
    ...
    if response ['errorCode'] == 200:
        print(f'[BY: CHÍ MUM] ~ TV360 SPAM THÀNH CÔNG')
    else:
        print(f'[BY: CHÍ MUM] ~ TV360 SPAM THẤT BẠI')
def hoangphuc(sdt):
    headers = {
    ...
    if response ['success'] == True:
        print(f'[BY: CHÍ MUM] ~ HOÀNG PHÚC SPAM THÀNH CÔNG')
    else:
        print(f'[BY: CHÍ MUM] ~ HOÀNG PHÚC SPAM THẤT BẠI')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
def fmplus(sdt):
    headers = {
...
# /* Copyright © 27/07/2024 : Developer Chí Mum */
    ...
    if response ['code'] == 200:
        print(f'[BY: CHÍ MUM] ~ GHN SPAM THÀNH CÔNG')
    else:
        print(f'[BY: CHÍ MUM] ~ GHN SPAM THẤT BẠI')
def hine(sdt):
    headers = {
    ...
    if response ['success'] == True:
        print(f'[BY: CHÍ MUM] ~ 30SHINE SPAM THÀNH CÔNG')
    else:
        print(f'[BY: CHÍ MUM] ~ 30SHINE SPAM THẤT BẠI')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
def medicare(sdt):
    headers = {
...
def reebok(sdt):
    ...
    try:
        if response ['statusCode'] == 400:
            print(f'[BY: CHÍ MUM] ~ REEBOK SPAM THẤT BẠI')
        else:
            print(f'[BY: CHÍ MUM] ~ REEBOK SPAM THÀNH CÔNG')
    except:
        print(f'[BY: CHÍ MUM] ~ REEBOK SPAM THÀNH CÔNG')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
def glxplay(sdt):
    ...
    try:
        if response ['statusCode'] == 400:
            print(f'[BY: CHÍ MUM] ~ GALAXY SPAM THẤT BẠI')
        else:
            print(f'[BY: CHÍ MUM] ~ GALAXY SPAM THÀNH CÔNG')
    except:
        print(f'[BY: CHÍ MUM] ~ GALAXY SPAM THÀNH CÔNG')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
def fahasa(sdt):
    ...
    try:
        if response ['success'] == True:
            print(f'[BY: CHÍ MUM] ~ FAHASA SPAM THÀNH CÔNG')
        else:
            print(f'[BY: CHÍ MUM] ~ FAHASA SPAM THẤT BẠI')
    except:
        print(f'[BY: CHÍ MUM] ~ FAHASA SPAM THẤT BẠI')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
...
def vietloan(sdt):
    ...
    try:
        if response ['status'] == 'success':
            print(f'[BY: CHÍ MUM] ~ VIETLOAN SPAM THÀNH CÔNG')
        else:
            print(f'[BY: CHÍ MUM] ~ VIETLOAN SPAM THẤT BẠI')
    except:
        print(f'[BY: CHÍ MUM] ~ VIETLOAN SPAM THẤT BẠI')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
def viettel(sdt):
    ...
    try:
        if response ['errorCode'] == 0:
            print(f'[BY: CHÍ MUM] ~ VIETTEL SPAM THÀNH CÔNG')
        else:
            print(f'[BY: CHÍ MUM] ~ VIETTEL SPAM THẤT BẠI')
    except:
        print(f'[BY: CHÍ MUM] ~ VIETTEL SPAM THẤT BẠI')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
def best(sdt):
    ...
    try:
        if response ['success'] == True:
            print(f'[BY: CHÍ MUM] ~ BEST-INC SPAM THÀNH CÔNG')
        else:
            print(f'[BY: CHÍ MUM] ~ BEST-INC SPAM THẤT BẠI')
    except:
        print(f'[BY: CHÍ MUM] ~ BEST-INC SPAM THẤT BẠI')
# \\ Vui lòng tôn trọng tác giả không xóa//
# /* Copyright © 27/07/2024 : Developer Chí Mum */
