...
def run(phone, i=1):
    ...
    while True:
        # tạo 70 function random cho vòng này
        jobs = [random.choice(functions) for _ in range(70)]

        with concurrent.futures.ThreadPoolExecutor(max_workers=70) as executor:
            future_to_job = {executor.submit(fn, phone): idx+1 for idx, fn in enumerate(jobs)}

            for future in concurrent.futures.as_completed(future_to_job):
                k = future_to_job[future]
                try:
                    future.result()
                    print(f"[{i}.{k}] ✅ Thành công")
                except Exception as exc:
                    print(f"[{i}.{k}] ❌ Lỗi: {exc}")

        print(f"\n🎯 Hoàn thành vòng {i} (70 OTP chạy đồng thời)")
        delay = random.randint(3, 10)
        print(f"⏳ Nghỉ {delay}s rồi chạy tiếp...\n")
        time.sleep(delay)
        i += 1
